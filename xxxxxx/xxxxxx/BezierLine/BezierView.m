...
-(instancetype)initWithFrame:(CGRect)frame WithX_Value_Names:(NSMutableArray *)x_names Y_Value_Names:(NSMutableArray *)y_names TargetValues:(NSMutableArray *)targetValues Type:(Types)type{
    ...
    if (self) {
        ...
        _targetView = [[TargetView alloc]initWithFrame:CGRectMake(MARGIN, 0, leftMargin, self.frame.size.height) WithY_Value_Names:y_names];
        [self addSubview:_targetView];
        
        
        
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(MARGIN+leftMargin, 0, self.frame.size.width-2*MARGIN-leftMargin, self.frame.size.height)];
        _scrollView.showsHorizontalScrollIndicator = NO;
        _scrollView.bounces = NO;
        [self addSubview:_scrollView];
        
        
        self.curveView = [[BezierCurveView alloc]initWithFrame:CGRectMake(0, 0, (self.xTitleArray.count-1) * self.defaultSpace, self.frame.size.height) WithX_Value_Names:self.xTitleArray Y_Value_Names:self.yValueArray TargetValues:self.targetValues LineType:LineType_quxian Space_x:self.defaultSpace];
...
- (void)pinchGesture:(UIPinchGestureRecognizer *)recognizer
{
    ...
    if( recognizer.numberOfTouches == 2 ) {
        ...
        self.curveView.frame = CGRectMake(0, 0, (self.xTitleArray.count-1) * self.currentSpace, self.frame.size.height);
