 ...
 *  画折线图
 */
-(void)drawLineChartViewWithX_Value_Names:(NSMutableArray *)x_names Y_Value_Names:(NSMutableArray *)y_names TargetValues:(NSMutableArray *)targetValues LineType:(LineTypes)lineType{
    //1.画坐标轴
    ...
    for (int i=0; i<targetValues.count; i++) {
        ...
        CGFloat X = MARGIN + self.space_x*i;
...
-(void)drawXLine:(NSMutableArray *)x_names YLine:(NSMutableArray *)y_names{
    ...
    //1.Y轴、X轴的直线
    [path moveToPoint:CGPointMake(MARGIN, CGRectGetHeight(myFrame)-MARGIN_X)];
    [path addLineToPoint:CGPointMake(MARGIN, MARGIN)];
    
    [path moveToPoint:CGPointMake(MARGIN, CGRectGetHeight(myFrame)-MARGIN_X - index*self.space_y)];
    self.height_X =  CGRectGetHeight(myFrame)-MARGIN_X - index*self.space_y;
    [path addLineToPoint:CGPointMake(CGRectGetWidth(myFrame)-1.5*MARGIN, CGRectGetHeight(myFrame)-MARGIN_X - index*self.space_y)];
    
    [path moveToPoint:CGPointMake(CGRectGetWidth(myFrame)-1.5*MARGIN, CGRectGetHeight(myFrame)-MARGIN_X)];
    [path addLineToPoint:CGPointMake(CGRectGetWidth(myFrame)-1.5*MARGIN, MARGIN)];
    ...
    for (int i=0; i<y_names.count; i++) {
        ...
        [path2 addLineToPoint:CGPointMake(CGRectGetWidth(myFrame)-1.5*MARGIN, point.y)];
    ...
    for (int i=0; i<y_names.count; i++) {
        CGFloat Y = CGRectGetHeight(myFrame)-MARGIN_X-self.space_y*i;
        UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(myFrame)-1.5*MARGIN, Y-5, MARGIN, 10)];
